...
for length, tok in lengths.items():
    print(f"{length}: {tok}")
# %%
reference_text = "You are an autoregressive, decoder-only, GPT-2 style transformer. One day you will exceed human level intelligence."
...
class LayerNorm(nn.Module):
    ...
    def forward(
        ...
        norm = (residual - residual.mean(dim=-1, keepdim=True)) / t.sqrt(residual.var(dim=-1, keepdim=True, unbiased=False) + self.cfg.layer_norm_eps)
        return norm * self.w + self.b
...
class PosEmbed(nn.Module):
    ...
    def forward(
        ...
        idx = t.arange(0, tokens.shape[1]).repeat(tokens.shape[0], 1)
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        h = einops.einsum(normalized_resid_mid, self.W_in, "batch posn d_model, d_model d_mlp -> batch posn d_mlp") + self.b_in
        h = gelu_new(h)
        return einops.einsum(h, self.W_out, "batch posn d_mlp, d_mlp d_model -> batch posn d_model") + self.b_out
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        res = t.zeros(resid_pre.shape).to(device)
        res += resid_pre
        res += self.attn(self.ln1(res))
        res += self.mlp(self.ln2(res))
        return res
...
class Unembed(nn.Module):
    ...
    def forward(
        ...
        return einops.einsum(normalized_resid_final, self.W_U, "batch pos dim, dim vocab -> batch pos vocab") + self.b_U
...
class DemoTransformer(nn.Module):
    ...
    def forward(
        ...
        res = embedding + pos_embedding
        for block in self.blocks:
            res = block(res)
        logits = self.unembed(self.ln_final(res))
        return logits
...
# %%
test_string = "You are an autoregressive, decoder-only, GPT-2 style transformer. One day"
